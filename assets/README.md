# Third-party assets

The following image files are redistributed under the GNU General Public
License, version 3 or later. The license text is in
[`LICENSES/GPL-3.0-or-later.txt`](../LICENSES/GPL-3.0-or-later.txt).

## `gnu/meditate-fs.jpg`

- Title: *Levitating, Meditating, Flute-playing Gnu*
- Artist: Nevrax Design Team
- Copyright: © 2001, 2004 Free Software Foundation, Inc.
- Source: <https://www.gnu.org/graphics/meditate.en.html>
- Historical GNU mirror: <http://gnu.ist.utl.pt/graphics/meditate.html>
- License: GPL-2.0-or-later (this repository distributes it under
  GPL-3.0-or-later); the source also offers GFDL-1.1-or-later
- Modification: none; this file is byte-for-byte identical to the GNU source

## `../config/doom/Emacs2.png`

- Title: GNU Emacs logo
- Author: Nicolas Petton
- Copyright: © 2015-2025 Free Software Foundation, Inc.
- Source: <https://git.savannah.gnu.org/cgit/emacs.git/tree/etc/images/icons/README>
- License: GPL-3.0-or-later
- Modification: separately encoded 128×128 copy of the official GNU Emacs logo

All other files in this repository remain subject to their own stated terms.
