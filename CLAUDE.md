...
Orchestrates: profile isolation, prerequisites (via `scripts/install-prerequisites.ps1`), Doom Emacs, hack worktree tooling, and Claude Code setup.
...
- **`claude-code`** — Claude Code CLI in a vterm side buffer (`stevemolitor/claude-code.el`); opens on the right when the frame is ≥160 cols, otherwise on the bottom.
...
`~/.claude/skills/`, and TRAMP support. Sub-agent calls are routed to local Ollama so
...
| `scripts/install-claude.ps1` | Windows: toast notifications for Claude Code |
