...
### 1. LSP Client: lsp-mode (Doom's default)

**Choice:** bare `:tools lsp` — no `+eglot` flag, so lsp-mode + lsp-ui

**Rationale:**
- It is what Doom ships by default, so it is the best-tested path through every `+lsp`
  language module and needs no per-language re-wiring
- Language modules gate features on it: `:lang python +pyright` pulls `lsp-pyright` only
  under `:tools lsp -eglot`
- Richer out of the box (lsp-ui, `lsp-install-server`, workspace configuration per client)

eglot was the original choice here — built into Emacs core since v29, minimal, no external
dependencies. Trading that for the default path was judged worth it: staying on Doom's
default is itself a long-term-support argument, since it is the configuration upstream
exercises. See the revision note at the end of this ADR.
...
- Language tooling choices otherwise favor GNU/official project maintenance over feature-rich
  community alternatives
- Code navigation: LSP provides go-to-definition; ctags (`SPC c t`) remains as fallback
...
**Still stale, not yet reconciled:** the Roslyn subsections below section 1 describe
